	...
	where TResponse : notnull
{
	...
	/// not an error).
	/// </summary>
	public override async Task<ValidationResult> ValidateAsync(ValidationContext<TCommand> context, CancellationToken cancellation = default)
	{
		List<ValidationFailure> failures = [];
		foreach (var validator in validators)
		{
			var result = await validator.ValidateAsync(context.InstanceToValidate.Request, cancellation).ConfigureAwait(false);
