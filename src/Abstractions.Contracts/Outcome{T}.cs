...
public sealed class Outcome<T> : IUnion where T : notnull
{
	...
	///     <paramref name="value" /> carries a null <see cref="Problem" /> — a smuggled
	///     default <see cref="Failed" /> past the case type's own guard.
	/// </exception>
	...
	[SuppressMessage("Design", "CA1000:Do not declare static members on generic types",
		Justification =
			"Outcome<T>.Ok/Err are the type's only construction path by design — an instance-side factory would need an already-constructed Outcome<T> to call it from.")]
	...
	public static Outcome<T> Err(ErrorCategory category, IReadOnlyDictionary<string, string[]>? errors = null,
		Guid? correlationId = null, ErasureReceipt? receipt = null) =>
		new(new Failed(new Problem
		{
			Category = category,
	...
	[SuppressMessage("Design", "CA2225:Operator overloads have named alternates",
		Justification =
			"Ok(T) is the named alternate — this operator exists purely as ergonomic sugar over it, a second method with a different name would be pure duplication.")]
	public static implicit operator Outcome<T>(T value) => value is null ?
		null! :
		Ok(value);
	...
	[SuppressMessage("Design", "CA2225:Operator overloads have named alternates",
		Justification =
			"Match(...) is the named alternate — this operator exists purely as ergonomic sugar over it, a second method with a different name would be pure duplication.")]
	public static explicit operator T(Outcome<T> outcome) => outcome is null ?
		default! :
		outcome.Match(
			static ok => ok,
